...
Both survival and recruitment data can include multiple populations (different `PopulationName` values).
Survival data can also be provided as one row per population per year (aggregate annual data).
Placeholder rows with `NA` measurement columns can be added for unobserved years with `allow_missing = TRUE`.
See the [extensions article](https://poissonconsulting.github.io/bboutools/articles/extensions.html) for details.
...
See the [priors article](https://poissonconsulting.github.io/bboutools/articles/priors.html) for more information.
...
The user can also specify priors informed by national demographic-disturbance relationships [@johnson_science_2020] using `bb_priors_survival_national()` and `bb_priors_recruitment_national()`.
These functions return intercept priors based on the level of anthropogenic and fire disturbance.
...
If the user is interested in fitting models without any prior information, see `bb_fit_recruitment_ml()` and `bb_fit_survival_ml()`, which use a Maximum Likelihood approach (see [Maximum Likelihood](#maximum-likelihood) below).
...
See the [analytical methods article](https://poissonconsulting.github.io/bboutools/articles/methods.html) for details.

Predictions of calf-cow ratio can also be made using `bb_predict_calf_cow_ratio()`. 

The sex ratio is set at fit time via the `sex_ratio` argument in `bb_fit_recruitment()` and is automatically extracted by predict functions.
...
Details on how lambda is calculated can be found in the [analytical methods article](https://poissonconsulting.github.io/bboutools/articles/methods.html).
...
The function `bb_fit_survival()` auto-detects multiple populations from the `PopulationName` column.
...
See the [extensions article](https://poissonconsulting.github.io/bboutools/articles/extensions.html) for more information on multi-population analysis, aggregate annual survival data, predicting unobserved years and other extensions.
...
ML models do not currently support multi-population analysis, unobserved year predictions (`allow_missing`) or prior-only sampling.
These features are available only with the Bayesian fitting functions.
