...
The default prior distribution parameter values can be accessed from `bb_priors_recruitment()` and `bb_priors_survival()`. See documentation (i.e., `?bb_priors_recruitment`) for more information on the prior distributions.

```{r}
bb_priors_recruitment()
```
The default prior distribution for adult_female_proportion is `Beta(65, 35)` and the default prior distribution for the intercept (`b0`) is `Normal(-1.4, 0.5)`, which is on the log-odds scale. 
...
For example, uninformative priors for `adult_female_proportion` (e.g., `Beta(1, 1)`) and `b0` (e.g., `Normal(0, 2)`) can be supplied as follows. 
```{r, eval=FALSE}
recruitment <- bb_fit_recruitment(bboudata::bbourecruit_a, priors = c(adult_female_proportion_alpha = 1, adult_female_proportion_beta = 1, b0_mu = 0, b0_sd = 2))
```
