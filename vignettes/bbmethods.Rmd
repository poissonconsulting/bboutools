...
data <- bboutools:::model_data_survival(surv, year_start = 4L, include_uncertain_morts = FALSE, quiet = FALSE)$data
obs <- 
  data %>%
  group_by(Year, Month) %>%
  summarise(survival = 1 - (MortalitiesCertain)/StartTotal,
            weight = StartTotal^(1/2)) %>%
  ungroup() %>%
  group_by(Year) %>%
  summarise(mean_survival = weighted.mean(survival, weight)^12,
            mean_weight = mean(weight))

fit <- bb_fit_survival_ml(surv, quiet = TRUE)
fit2 <- bb_fit_survival_ml(surv, min_random_year = Inf, quiet = TRUE)
fit3 <- bb_fit_survival(surv, min_random_year = Inf, nthin = 50, quiet = TRUE)
fit4 <- bb_fit_survival(surv, nthin = 20, quiet = TRUE)
...
df <- dplyr::bind_rows(list("ML Random" = pred, 
                            "ML Fixed" = pred2,
                            "Bayes Fixed" = pred3,
                            "Bayes Random" = pred4), .id = "model") %>%
  mutate(method = if_else(grepl("ML", model), "Frequentist", "Bayesian"),
         year = if_else(grepl("Fixed", model), "Fixed", "Random"))

gp <- 
  ggplot(data = df) +
  geom_pointrange(aes(x = CaribouYear, 
                      y = estimate, 
                      ymin = lower, 
                      ymax = upper, 
                      color = model,
  ...
  geom_point(data = obs, aes(x = Year,
                             y = mean_survival,
                             alpha = mean_weight), 
             color = "black") +
...
In general, Maximum Likelihood and Bayesian models of the same type (i.e., fixed or random) have similar estimates. 
This is because the priors in the Bayesian models are not informative. 
Estimates from random effect models tend to be pulled toward the mean. 
Estimates from fixed effect models more closely match the observed data. 
